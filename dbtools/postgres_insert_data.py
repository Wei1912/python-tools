...
#
# insert data quickly for PostgreSQL
# read original data from text, excel
##################################################
...
def format_data(fields, row, types, is_excel):
    ...
    while i < len(fields):
        if row[i]:
            t = types[fields[i]]
            if t in ['character', 'character varying']:
                row[i] = str(row[i])
            elif t in ['integer', 'int']:
                row[i] = int(row[i])
            elif t in ['real', 'double precision']:
                row[i] = float(row[i])
...
def create_query(schema, table, fields):
    query_part1 = 'INSERT INTO "' + schema + '"."' + table + '" ('
    for f in fields:
        query_part1 = query_part1 + '"' + f + '",'
    if query_part1.endswith(','):
        query_part1 = query_part1[:-1]
    query_part1 = query_part1 + ') VALUES '
    query_part2 = '(' + ','.join(['%s'] * len(fields)) + ')'
...
def process():
    ...
    query_part1, query_part2 = create_query(glb_db_schema, glb_db_table, headers)
    print('query:')
    print(query_part1)
    print(query_part2)
    print()
